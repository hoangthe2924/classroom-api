...
module.exports = {
    ...
    async updateAssignment(assignment){
        try {
            const result = await Assignment.update({title: assignment.title, point: assignment.point},{where: {id: assignment.id}, returning: true});
    ...
    async deleteAssignment(classID, assignmentID){
        try {
            const result = await Assignment.destroy({where: {id: assignmentID, classId: classID}});
            return result? result:false;
        } catch (error) {
            console.log(error);
            return false;
    ...
    async getListAssignment(classID){
        try {
            return Assignment.findAll({where: {classId: classID}, order: [['order','ASC']], attributes: ['id','title','point','order']});
        } catch (error) {
            console.log(error);
            return false;
        }
    ...
    async updateAssignmentOrder(classID, assignments){
        try {
            for (let assignment of assignments){
                Assignment.update({order: assignment.order},{where: {id:assignment.id, classId: classID}});
            }
            return true;
        } catch (error) {
            console.log(error);
